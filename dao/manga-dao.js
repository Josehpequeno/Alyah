...
class MangaDao {
    ...
    createManga(name, description, author, cover) {
        return new Promise((resolve, reject) => {
            let authorDao = new AuthorDao(this._db);
            authorDao.getIdAuthor(author).then(author_id => {
                this._db.query(
                    'INSERT INTO mangas (name, description, author_id, cover) VALUES ($1, $2, $3, $4) RETURNING *',
                    [name, description, author_id, cover],
                    (error, results) => {
                        if (error) {
                            return reject(error);
                        }
                ...
                if (err == "No results!") {
                    authorDao.createAuthor(author).then(author_id => {
                        this._db.query(
                            'INSERT INTO mangas (name, description, author_id, cover) VALUES ($1, $2, $3, $4) RETURNING *',
                            [name, description, author_id, cover],
                            (error, results) => {
